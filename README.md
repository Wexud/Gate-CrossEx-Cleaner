...
## Крок 2. Відкрийте Windows PowerShell прямо в папці зі скриптом
...
> Не вводьте просто `powershell`. У деяких системах Провідник може сприйняти `PowerShell` як назву папки та перейти, наприклад, у `Документи\PowerShell` замість запуску програми.

Після `powershell.exe` відкриється Windows PowerShell уже в папці зі скриптом.
...
3. введіть **`powershell.exe`**;
4. натисніть Enter;
5. переконайтесь, що PowerShell відкрився саме в папці `Gate-CrossEx-Cleaner`;
6. повторіть команду запуску.
...
Якщо у вас встановлений PowerShell 7:

1. відкрийте папку `Gate-CrossEx-Cleaner` у Провіднику;
2. клікніть по рядку адреси;
3. введіть **`pwsh.exe`**;
4. натисніть Enter.
