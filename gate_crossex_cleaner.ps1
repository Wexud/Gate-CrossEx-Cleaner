...
function RoundDown([decimal]$n,[int]$p){if($p-lt0-or$p-gt28){throw "Invalid precision: $p"};$f=[decimal]1;1..$p|ForEach-Object{$f*=10};[decimal]::Floor($n*$f)/$f}
...
try {
    ...
    if($MaxQuoteWorseningPercent-lt0-or$StablecoinMaxLossPercent-lt0){throw 'Percentage limits cannot be negative'}
    if($MaxCandidates-lt1-or$MaxCandidates-gt40){throw 'MaxCandidates must be 1..40'}
    ...
    if($pre.Count -and (Yes 'Execute accepted Flash Swaps? Type YES')){
        foreach($p in $pre){
            $cur=Account;$x=Asset $cur $p.Venue $p.Coin;if($null-eq$x){continue};$n=D $x.available_balance 'current available_balance';if($n-le0){continue}
    ...
    $u=UsdtAvailable $a;if($u-le0){Write-Host 'No CROSSEX USDT available for SPOT transfer.';return}
    $rule=TransferRule;if($null-eq$rule){throw 'No USDT transfer rule returned'};if([int]$rule.is_disabled-ne0){Write-Host 'USDT transfer is disabled by Gate.' -ForegroundColor Yellow;return}
    $prec=[int]$rule.precision;$min=D $rule.min_trans_amount 'min_trans_amount';$fee=D $rule.est_fee 'est_fee';$base=$u;if($fee-gt0){$base-=$fee};if($base-lt0){$base=0};$amt=RoundDown $base $prec
    ...
    if($amt-le0-or$amt-lt$min){Write-Host 'Transfer amount below minimum.';return};if(!(Yes "Transfer $(DS $amt) USDT to SPOT? Type YES")){Write-Host 'Transfer cancelled.';return}
